...
TMC2209ReadResp_t query(const TMC2209ReadReq_t *req) {
    ...
    uart_write_blocking(uart1, b1.buffer, 4); 

    Buffer4Byte_t b2;
    uart_read_blocking(uart1, b2.buffer, 4);

    Buffer8Byte_t b3;
    uart_read_blocking(uart1, b3.buffer, 8);
...
void modify(const TMC2209Write_t *w) {
    ...
    uart_write_blocking(uart1, b1.buffer, 8);

    Buffer8Byte_t b2;
    uart_read_blocking(uart1, b2.buffer, 8);
...
struct Register<SG_RESULT_t> {
    ...
    static uint32_t serialize(const SG_RESULT_t *d) {
        return d->result;
    }
...
struct Register<SGTHRS_t> {
    ...
    static uint32_t serialize(const SGTHRS_t *d) {
        return d->threshold;
    }
...
struct Register<TCOOLTHRS_t> {
    ...
    static uint32_t serialize(const TCOOLTHRS_t *d) {
        return d->threshold;
    }
...
struct Register<TSTEP_t> {
    ...
    static uint32_t serialize(const TSTEP_t *d) {
        return d->result;
    }
